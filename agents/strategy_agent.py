...
class StrategyAgent:
    ...
    def _build_vlm_agent(self, strategies: List[Dict]):
        ...
        system_prompt = (
            ...
            "- NUMERIC FRAGMENTS: If a numeric text item (e.g., '8765') appears to be part of a larger "
            "sensitive number (bank account, SSN, credit card), that is a reason to KEEP or STRENGTHEN "
            "protection, NOT remove it. Being part of a larger sensitive number makes it MORE dangerous, "
            "not less. Never reason that 'X is part of Y, so X does not need protection.'\n\n"
